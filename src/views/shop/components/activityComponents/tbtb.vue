<template>
  <div class="form">
    ...
    <div class="form-item">
      <span class="form-required">*必填</span>
      <span class="form-label">选择活动类型：</span>
      <div class="form-block radio-list">
        <el-radio-group
          v-model="form.guideEntrance"
          class="item"
          style="padding-top: 10px;"
          fill="red"
          text-color="#6666FF"
        >
          <el-radio v-for="item in hdList" :label="item.name">{{ item.name }}</el-radio>
        </el-radio-group>
      </div>
    </div>
    ...
    <div class="form-item">
      <span class="form-required">*必填</span>
      <span class="form-label">设置引导图：</span>
      <div class="form-err form-err-change"> （最多可添加3张图片，单张图片大小在3M以内选择文件）</div>
    </div>
    <div class="form-item">
      <span class="form-required"/>
      <div style="position: relative">
        <el-upload
          ...
          :http-request="uploadImageydt1"
          :show-file-list="false"
        >
          <img v-if="form.guideImgUrl1" :src="form.guideImgUrl1" class="avatar">
          <i v-else class="el-icon-plus avatar-uploader-icon"/>
        </el-upload>
        <i class="el-icon-circle-close avatar-uploader-remove" @click="form.guideImgUrl1=null"/>
      </div>
      <div style="position: relative">
        <el-upload
          style="margin-right: 20px;background-color: #F2F2F2"
          class="avatar-uploader"
          action
          :http-request="uploadImageydt2"
          :show-file-list="false"
        >
          <img v-if="form.guideImgUrl2" :src="form.guideImgUrl2" class="avatar">
          <i v-else class="el-icon-plus avatar-uploader-icon"/>
        </el-upload>
        <i class="el-icon-circle-close avatar-uploader-remove" @click="form.guideImgUrl2=null"/>
      </div>
      <div style="position: relative">
        <el-upload
          style="margin-right: 20px;background-color: #F2F2F2"
          class="avatar-uploader"
          action
          :http-request="uploadImageydt3"
          :show-file-list="false"
        >
          <img v-if="form.guideImgUrl3" :src="form.guideImgUrl3" class="avatar">
          <i v-else class="el-icon-plus avatar-uploader-icon"/>
        </el-upload>
        <i class="el-icon-circle-close avatar-uploader-remove" @click="form.guideImgUrl3=null"/>
      </div>
    </div>
    <div class="form-item">
      <span class="form-required">*必填</span>
      <span class="form-label">设置单数：</span>
      <el-input-number v-model="form.orderNum" size="mini" :min="0" style="width: 130px">
      </el-input-number>
    ...
    <div v-if="expressFlag" class="form-item">
      ...
      <el-input v-model="form.taskAdditionalIncrement.weight" size="small" style="width: 150px">
    ...
    export default {
        ...
        methods: {
            init() {
                ...
                selectYwTaskActivityTypeList({storeTypeId: this.storeType}).then(response => {
            ...
            uploadImageydt1(e) {
                this.form.guideImgUrl1 = uploadImage(e)
            },
            uploadImageydt2(e) {
                this.form.guideImgUrl2 = uploadImage(e)
            },
            uploadImageydt3(e) {
                this.form.guideImgUrl3 = uploadImage(e)
            },
            uploadImage1(e) {
            ...
            handleNext() {
                if (this.orderAll != this.form.orderNum) {
                    this.$message({
                        message: '发布的单量不正确',
                        type: 'warning'
                    })
            ...
            handleChangeReleaseDate(val) {
                ...
                if (val) {
                    this.orderAll = this.form.orderNum
                    var myDate = new Date()
                    var month = myDate.getMonth() < 9 ? '0' + (myDate.getMonth() + 1) : myDate.getMonth() + 1
                    var day = myDate.getDate() < 9 ? '0' + (myDate.getDate()) : myDate.getDate()
                    var dataToDate = myDate.getFullYear() + '-' + month + '-' + day
